...
function openRoomModal(id) {
    ...
    el.querySelectorAll('.rm-error').forEach(e => { e.style.display = 'none'; e.textContent = ''; });
}
...
function _renderRoomExpList() {
    ...
    if (!matched.length) { listEl.innerHTML = '<div class="room-exp-bldg-empty">No buildings match your search.</div>'; return; }
...
function _updateRoomExpSelCount() {
    ...
    if (allCb) { const total = _roomExpBuildings.length; allCb.checked = n === total && total > 0; allCb.indeterminate = n > 0 && n < total; }
...
function _updateRoomExpFooter() {
    ...
    const btn = document.getElementById('roomExpBtnLabel'); if (btn) btn.textContent = n > 0 ? `Export ${n} Building${n===1?'':'s'}` : 'Export';
    const sumEl = document.getElementById('roomExpSummaryText');
    if (sumEl) {
        const roomCount = _roomExpBuildings.filter(b => _roomExpSelectedIds.has(b.buildingid)).reduce((s, b) => s + (b.room_count || 0), 0);
        sumEl.innerHTML = (n===0||nf===0) ? 'Select buildings and formats to see export summary'
    ...
    const info = document.getElementById('roomExpFooterInfo'); if (info) info.textContent = fmts.join(', ');
}
...
function _hideRoomExpLoading() { document.getElementById('roomExpLoadingOverlay').style.display = 'none'; }
...
function _roomExportCSV(buildings, filename) {
    const now=new Date().toLocaleString(), q=v=>`"${String(v??'').replace(/"/g,'""')}"`;
...
async function roomExecuteExport(){
    ...
    const filename=_buildRoomFilename(),ids=Array.from(_roomExpSelectedIds);
    const btn=document.getElementById('roomExpConfirmBtn');btn.disabled=true;
    closeRoomExportModal();_showRoomExpLoading('Fetching Room Data','Loading room details from database...');
    let buildings;
    try{
