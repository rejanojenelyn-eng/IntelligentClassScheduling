// ── Export routes ─────────────────────────────────────────────────────────────
const _ARCH_DOCX_ROUTE = '/archived-employee/export/docx';
const _ARCH_XLSX_ROUTE = '/archived-employee/export/xlsx';

// ── Restore modals ────────────────────────────────────────────────────────────
function openRestoreModal(archiveId, name) {
    document.getElementById('restoreEmployeeName').textContent = name;
    document.getElementById('confirmRestoreBtn').href = `/restore_employee/${archiveId}`;
...
document.getElementById('confirmBulkRestoreBtn').addEventListener('click', function () {
    ...
    fetch('/employee/bulk_restore', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ archive_ids: ids }),
    }).then(r => r.json()).then(data => {
        if (data.success) window.location.reload();
        else { alert(data.message || 'Error restoring employees.'); this.disabled = false; this.textContent = 'RESTORE ALL'; }
    }).catch(() => { alert('Network error.'); this.disabled = false; this.textContent = 'RESTORE ALL'; });
});
