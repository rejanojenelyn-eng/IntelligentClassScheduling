...
async function analyzeEmpCsv() {
    ...
    try {
        const res  = await fetch('/faculty/import/csv/analyze', { method: 'POST', body: fd });
...
async function analyzeEmpXlsx() {
    ...
    fd.append('file', fileInput.files[0]);
    try {
        const res  = await fetch('/faculty/import/xlsx/analyze', { method: 'POST', body: fd });
        const data = await res.json();
        hideLoading();
...
async function analyzeEmpPdf() {
    ...
    fd.append('file', fileInput.files[0]);
    try {
        const res  = await fetch('/faculty/import/pdf/analyze', { method: 'POST', body: fd });
        const data = await res.json();
        hideLoading();
...
async function analyzeEmpDocx() {
    ...
    fd.append('file', fileInput.files[0]);
    try {
        const res  = await fetch('/faculty/import/docx/analyze', { method: 'POST', body: fd });
        const data = await res.json();
        hideLoading();
...
// ── EXPORT ENGINE ────────────────────────────────────────────────────────────
const _EMP_DOCX_ROUTE = '/employee/export/docx';
...
function _rowsToEmpData(rows) {
    // Acad head table has no Contact column (8 cells)
    return rows.map(row => ({
...
function _exportCSV(data, filename) {
    ...
    const headers = ['Employee Number', 'Employee Name', 'Specialization', 'Email', 'Employment Type', 'Status'];
    const dataRows = data.map(e => [e.emp_num, e.name, e.spec, e.email, e.type, e.status]);
...
async function _exportXLSX(data, filename) {
    ...
    const res = await fetch('/employee/export/xlsx', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
window.onclick = function (event) {
    ["addEmployeeModal", "archiveConfirmModal", "bulkArchiveConfirmModal",
     ...
     "empPdfModal", "empDocxModal", "empReviewModal", "facultyPopupModal"].forEach(id => {
...
async function _loadFacultySchedule(empNum) {
    ...
    try {
        ...
        if (ACAD_ACTIVE_AY_ID) params.set('ay_id', ACAD_ACTIVE_AY_ID);
        if (ACAD_ACTIVE_SEM)   params.set('semester', ACAD_ACTIVE_SEM);
...
async function _loadFacultyLoad(empNum) {
    ...
    try {
        const params = new URLSearchParams({ emp_num: empNum });
        if (ACAD_ACTIVE_AY_ID) params.set('ay_id', ACAD_ACTIVE_AY_ID);
        if (ACAD_ACTIVE_SEM)   params.set('sem', ACAD_ACTIVE_SEM);
